	...
	*/
	...
	public function wpmenucart_menu_item() {
		...
		if ($item_data['cart_contents_count'] == 0 && ( !isset($this->options['always_display']) ) ) {
			$empty_menu_item = '<a class="wpmenucart-contents awemenu-icon menu-shopping-cart" style="display:none">&nbsp;</a>';
		...
		if ($item_data['cart_contents_count'] == 0) {
			...
			$menu_item_classes = 'wpmenucart-contents empty-wpmenucart-visible awemenu-icon menu-shopping-cart';
		} else {
			...
			$menu_item_classes = 'wpmenucart-contents awemenu-icon menu-shopping-cart';
		}
		...
		if (isset($this->options['icon_display'])) {
			...
			$menu_item_icon = '<div class="icon icon-cart"></div>';
			$menu_item_a_content .= $menu_item_icon;
		}
		
		switch ($this->options['items_display']) {
			case 1: //items only
				$menu_item_a_content .= '<div class="text"><p>My Cart</p><p class="cart-number">'.$cart_contents.' - <span class="price">'.$item_data['cart_total'].'</span></p></div>';
				break;
			case 2: //price only
	...
	public function wpmenucart_submenu_item() {
		...
		// Check empty cart settings
		if ($item_data['cart_contents_count'] == 0 && ( !isset($this->options['always_display']) ) ) {
			$empty_submenu_item = '<ul class="submenu megamenu empty-wpmenucart" style="display:none"></ul>';
		...
		*/
		$submenu_classes = apply_filters( 'wpmenucart_submenu_classes', 'whishlist' );
		...
		if (isset($this->options['flyout_display'])) {
			...
			if ( $submenu_data != '' ) {
				$cart_submenu = '<ul class="submenu megamenu"><li><div class="container-fluid"><ul class="' . $submenu_classes . '"' . (!empty($submenu_style) ? ' style="'.$submenu_style.'"' : '') . '>';
				...
				foreach ( $submenu_data as $submenu_item_data ) {
					...
					// The thumbnail
					$cart_submenu_item_content = '<div class="product-image"><a href="'. $submenu_item_data['item_permalink'].'" title="'.$submenu_item_data['item_name'].'">'.$submenu_item_data['item_thumbnail'].'</a></div>';

					// Item info wrapper
					$cart_submenu_item_content .= '<div class="product-body">';
						// Product Name
						...
						$cart_submenu_item_content .= '<div class="whishlist-name"><h3><a href="'. $submenu_item_data['item_permalink'].'" title="'.$submenu_item_data['item_name'].'">'.$submenu_item_data['item_name'].'</a></h3></div>';

						// price
						$cart_submenu_item_content .= '<div class="whishlist-price">
						<span>Price:</span><strong>'. $submenu_item_data['item_price'].'</strong></div>';	

						// Quantity
						$cart_submenu_item_content .= '<div class="whishlist-quantity">
                                                                <span>Quantity:</span>
                                                                <span>'.$submenu_item_data['item_quantity'].'</span>
                                                            </div>';


						$cart_submenu_item_content .= '</div>';
					...
					// Set <li> classes
					$cart_submenu_item_li_classes = apply_filters( 'wpmenucart_submenu_item_li_classes', '', $submenu_item_data );
					$this->menu_items['submenu']['items'][$i]['item_li_classes'] = $cart_submenu_item_li_classes;

					// Wrap in <li>
					$cart_submenu_item_li = '<li><div class="whishlist-item">'.$cart_submenu_item_content.'</div></li>';
				...
				$cart_submenu .= '</ul>';
				$cart_submenu .= '<div class="menu-cart-total">
                                                <span>Total</span>
                                                <span class="price">'.$item_data['cart_total'].'</span>
                                            </div>';
                $cart_submenu .='<div class="cart-action">
                                                <a href="'.$cart_url.'" title="" class="btn btn-lg btn-dark btn-outline btn-block">View cart</a>
                                                <a href="/checkout/" title="" class="btn btn-lg btn-primary btn-block">Proceed To Checkout</a>
                                            </div>';
                $cart_submenu .= '</div></li></ul>';
			} else {
	...
	public function shortcode($atts) {
		...
		$menu = $before . '<li class="menubar-cart reload_shortcode">'.$this->wpmenucart_menu_item() . $this->wpmenucart_submenu_item() . '</li>' . $after;
		$html = '<ul class="navbar-icons wpmenucart-shortcode '.$flyout.'" style="'.$style.'">'.$menu.'</ul>';
		return $html;
