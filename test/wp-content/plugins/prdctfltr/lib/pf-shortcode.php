	...
	$('.prdctfltr-fix-wcsc:first').remove();
		...
		public static function get_products( $products ) {
			...
			if ( $products->have_posts() ) {
				...
				if ( $offset < 1 && isset( $prdctfltr_global['categories_active'] ) && $prdctfltr_global['categories_active'] === true ) {
					...
					if ( $show_categories == 'archive' ) {
						...
						add_filter( 'woocommerce_product_subcategories_hide_empty', __CLASS__ . '::fix_empty_cat_display' );
						woocommerce_product_subcategories( array( 'force_display' => true ) );
					}
					else if ( $show_categories == 'yes' || $show_categories == 'archive' ) {
